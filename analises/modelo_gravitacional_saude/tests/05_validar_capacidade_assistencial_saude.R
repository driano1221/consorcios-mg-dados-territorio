...
stopifnot(nrow(units) == 639L)
stopifnot(all(units$consulta_cnes_completa))
stopifnot(all(is.na(units$modulos_com_erro)))
stopifnot(sum(!units$unidade_movel_ou_itinerante) == 366L)
stopifnot(sum(units$unidade_movel_ou_itinerante) == 273L)
...
stopifnot(sum(entities$capacidade_status == "capacidade_direta_cnes_atual") == 46L)
stopifnot(sum(entities$capacidade_status == "sem_unidade_cnes_direta_nao_interpretar_como_zero") == 36L)
