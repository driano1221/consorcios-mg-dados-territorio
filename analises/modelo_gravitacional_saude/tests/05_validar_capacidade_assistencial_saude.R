...
stopifnot(sum(units$unidade_fixa_elegivel) == 82L)
stopifnot(sum(units$unidade_movel_pelo_tipo) == 586L)
stopifnot(sum(units$classificacao_fixa_pendente) == 2L)
# Regressao: USB/USA sem 'movel' no nome nunca pode virar destino fixo.
mobile_type <- grepl("movel|móvel", units$tipo_estabelecimento_cnes, ignore.case = TRUE)
stopifnot(!any(units$unidade_fixa_elegivel[mobile_type]))
stopifnot(all(!units$unidade_fixa_elegivel[units$classificacao_fixa_pendente]))
source_units <- read.csv(file.path(out_dir, "unidades_cnes_vinculadas_saude_mg.csv"),
  colClasses = "character", fileEncoding = "UTF-8")
source_index <- match(paste(units$cnpj_raiz_8, units$cnes), paste(source_units$cnpj_raiz_8, source_units$cnes))
stopifnot(!anyNA(source_index))
stopifnot(all(as.character(units$vinculo_cnpj_proprio_direto) == source_units$vinculo_cnpj_proprio_direto[source_index]))
stopifnot(all(as.character(units$vinculo_cnpj_mantenedora_direto) == source_units$vinculo_cnpj_mantenedora_direto[source_index]))
stopifnot(all(units$data_extracao_cnes == "2026-09-03"))
