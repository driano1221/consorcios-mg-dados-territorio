...
## 25/09: Comparação Dos Dois Modelos Na Aba Visual

A aba Modelo de `outputs/visuais_v1/index.html` apresenta os **resultados de 2019 já estimados**. O script `comparacao_modelos_visual.py`, chamado por `28_montar_visuais_v1.py`, lê os CSVs auditados, confere contagens e métricas, gera dois gráficos SVG/PNG e monta o texto. Ele não treina novamente, não altera a base v1 e preserva a apresentação anterior em seção recolhida. O ZIP local contém o HTML, figuras e manifesto com tamanho e SHA256 das fontes.

**Pipeline dos dados.** MIDES: somar pagamentos por município, ano 2019 e raiz CNPJ; positivo vira resposta 1, zero vira 0, permitindo vários 1 por município. O núcleo financeiro contém 73 consórcios. IBGE fornece população da origem em 2019. CNES, fotografia de dezembro, fornece horas SUS por unidade e CNPJ do consórcio. A matriz Distbrasil fornece quilômetros rodoviários entre sedes municipais; o destino é o município da unidade clínica em S1, a sede cadastral do consórcio em S2 e clínica quando disponível/sede nos demais em S3. A seleção passa por 54 consórcios com clínica e rota, 53 com horas clínicas positivas e 62 na ampliação com horas de outras modalidades e sede. Horas e rotas não foram imputadas. Profissionais, serviços e leitos SUS ficam fora desta especificação; RCL não cobre suficientemente o recorte; bacia foi adiada; população da sede não mede diretamente oferta assistencial.

**Auditoria antes de comparar.** O piloto original tinha 45.209 pares/771 pagos (53) e 52.886/1.299 (62). A decisão SICOM rejeitou a atribuição de Conselheiro Pena–CISVI/2019; essa resposta ficou indeterminada e a linha saiu do treino/teste, sem virar zero. São Francisco de Paula–CISMARG permaneceu positivo. O piloto binário foi reestimado pelo script 40 nas mesmas linhas e divisões do teste relativo: 45.208 pares/770 pagos em 53 candidatos e 52.885/1.298 em 62. O par contestado continua no denominador da atração, pois a dúvida recai sobre o pagamento, não sobre a oferta cadastrada. Os 853 municípios são mantidos em todos os cenários.

**EDA que orienta a leitura.** No recorte clínico, 770/45.208 pares (1,70%) têm pagamento. Entre os 853 municípios, 160 não pagam a nenhum dos 53 candidatos, 619 pagam a um e 74 a dois ou mais. No recorte ampliado, são 75, 322 e 456. A distância mínima mediana ao destino clínico é 52 km nos pares pagos e 491 km nos não pagos. Assim, muitos zeros são pares distantes na grade estadual; um bom escore pode refletir essa distinção relativamente fácil. As linhas selecionadas têm população, horas positivas e rota, porque esses são critérios do recorte, não por preenchimento artificial. Isso não demonstra que todos os 53/62 eram opções institucionais acessíveis.

**Modelo A, piloto binário auditado.** Seja `H_j` a soma de horas SUS do consórcio, `w_ju = h_ju/H_j` e `d_iu` a distância em km da origem à unidade. A impedância é `L_ij = Σ_u w_ju ln(1+d_iu)`. O GLM binomial estima `logit(p_ij) = α + β_P ln(P_i) + β_H ln(H_j) + β_D L_ij`, com `p_ij = Pr(pagamento_ij > 0)`. O sinal estimado de `β_D` foi negativo. Cada par município–consórcio é uma observação, não cada unidade. O piloto não inclui o conjunto concorrente em sua fórmula.

**Modelo B, teste de Paulo adaptado aos vínculos múltiplos.** Para cada candidato, `A_ij = H_j^b Σ_u w_ju/(1+d_iu)^γ` e `s_ij = A_ij/Σ_k A_ik`. O script 43 estima `logit(p_ij) = a + b_P(ln P_i − 10) + λ logit(s_ij)`; `b`, `γ` e `λ` são restringidos a valores não negativos e aprendidos apenas no treino. O deslocamento `−10` apenas recentra log população. As frações `s_ij` somam 1 dentro do município, mas **não são probabilidades de adesão exclusiva**; o logit produz probabilidades binárias que podem somar mais de 1 e acomodam vários pagamentos. A especificação de controle usa `ln(A_ij)` sem `Σ_k A_ik`, para verificar o efeito de comparar concorrentes. O ajuste numérico usa L-BFGS-B, duas inicializações e verificação de convergência/gradiente.

**Treino e avaliação.** O script 32 construiu cinco grupos de municípios com semente fixa e cinco grupos espaciais por k-means de coordenadas usadas no atlas, sem respostas. Em cada repetição, quatro grupos treinam e o quinto testa; nenhum município do teste está no treino. O script 40 reestimou A após a auditoria; o 43 treinou B e controle nas mesmas divisões. São 4 recortes × 2 validações × 5 grupos = 40 avaliações comparáveis por especificação. As previsões fora do treino geram Brier, logloss, precisão média e AUC. Brier é `média[(y−p)^2]`, portanto menor é melhor; precisão média resume a ordenação de pagamentos raros. A validação é espacial e municipal **dentro de 2019**, não teste de previsão temporal nem de causalidade.

| Recorte | Brier A → B, municípios | Brier A → B, espacial |
|---|---:|---:|
| S1 clínica, 53 | 0,008886 → 0,007084 | 0,009657 → 0,007206 |
| S2 sede, 53 | 0,008958 → 0,007184 | 0,009660 → 0,007296 |
| S2 sede, 62 | 0,013839 → 0,012408 | 0,015137 → 0,012724 |
| S3 misto, 62 | 0,013803 → 0,012351 | 0,015175 → 0,012685 |

B também reduziu logloss e aumentou precisão média nos oito pares recorte/validação; teve Brier menor nos 40 grupos. O controle de atração própria ficou próximo de A. Essas comparações são **dentro de cada recorte**: não interpretar o Brier absoluto de 53 versus 62 como uma competição na mesma amostra. Exemplo verificável: Igarapé pagou R$ 4.740.790,51 ao CISMEP; Betim tinha 1.417 horas SUS a 17,748 km, Brumadinho 234 horas a 23,104 km. `H = 1.651`, `L ≈ 2,9667`, `s = 73,15%`; as probabilidades fora do treino foram 96,72% (A) e 98,15% (B). Em Jacinto–CISRAL, sem pagamento, B errou mais: 95,11%, contra 40,76% em A.

**Limites para a reunião.** Pagamento MIDES não é filiação jurídica, uso de serviços ou fluxo de pacientes. Horas CNES não são produção; dezembro não representa cada mês. A rota municipal é estática e não mede deslocamento intramunicipal. A sede pode não ser polo assistencial nem ter vigência histórica confirmada. O conjunto 53/62 é operacional e pode conter consórcios indisponíveis institucionalmente para certos municípios, além de excluir oferta indireta e móvel mal localizada. Essas questões e erros individuais impedem escolher B como forma final apenas pelo ganho preditivo. A próxima decisão substantiva é quais candidatos devem entrar no denominador e em que condições.

Reprodução: `Rscript 31_preparar_cenarios_adesao.R`, `Rscript 32_estimar_adesao_financeira.R`, `Rscript 40_reestimar_piloto_auditado_sicom.R`, `python 43_testar_atracao_relativa_paulo.py`, `python 28_montar_visuais_v1.py`, seguidos dos testes 17, 24, 28 e 43. Os dados grandes ficam em `outputs/`; `outputs/atracao_relativa_paulo_2019/fontes.csv` e `outputs/visuais_v1/dados/manifesto.csv` registram hashes. A aba pode ser regenerada a partir dos resultados já presentes apenas com o script 28.
