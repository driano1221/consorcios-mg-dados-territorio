...
<h2 class="subsection" id="bin-casos">Quatro exemplos reais</h2><p class="lead">S1, previsão sem o município no treino. O valor observado e a probabilidade respondem a perguntas diferentes.</p><div class="binary-cases">'''
    ...
    notes=['A previsão é compatível com o pagamento observado. Não prova que todos os serviços foram usados nas duas clínicas.',
           ...
           'O portal repete o CNPJ associado ao CISVI, mas dois empenhos somam R$ 8.836,43 em multa tributária e taxa de radiodifusão. O pagamento continua sinalizado; não é vínculo consorcial comprovado.']
    for key,note in zip(keys,notes):
        ...
        body+=f'<article><h3>{escape(r["municipio"])} × {escape(r["entidade"])}</h3><dl><div><dt>Pagamento em 2019</dt><dd>R$ {fmt(r["valor_total"],2)}</dd></div><div><dt>Probabilidade prevista</dt><dd>{escape(label)}</dd></div></dl><div class="prob-track" aria-hidden="true"><span style="width:{p:.6f}%"></span></div><p>{escape(note)}</p></article>'
...
<div class="data-layers"><article><h3>CNES corrigido</h3><p>Dois consultórios de pessoa física foram retirados do CISMARG. As quatro estimativas principais ficaram iguais: as unidades tinham zero horas SUS. Rotas e mapas da v1 foram atualizados.</p></article><article><h3>Credores sob verificação</h3><p>17 pares-ano, R$ 1,35 milhão, têm nomes conflitantes. Dois pertencem a 2019. Mantivemos valores e alertas; retirar os pares foi apenas uma sensibilidade. Em Conselheiro Pena, dois objetos de despesa agora contradizem a atribuição ao CISVI.</p></article><article><h3>Mesmo município não garante pagamento</h3><p>Nos 57 pares intramunicipais do S1, 52 pagaram (91,23%), mas a previsão média é 99,07%. Esse excesso de confiança continua aberto.</p></article><article><h3>Conferência financeira parcial</h3><p>Piedade–CISMIRECAR/2021 confere: R$ 779.355,34 no portal e no MIDES. Ipatinga–CONSAÚDE/2018 diverge em R$ 137.077,88. As consultas de 2019 não localizaram esses credores, mas isso não prova ausência anual de pagamento.</p></article></div>
